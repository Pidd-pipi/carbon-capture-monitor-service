...
// (a comma separated key=value list). Without the variable an empty provider
// is returned so callers always receive a usable map.
func NewLabelsProvider() LabelsProvider {
	raw := strings.TrimSpace(os.Getenv("ALERT_DEFAULT_LABELS"))
	if raw == "" {
		return &envLabelsProvider{labels: map[string]string{}}
	}
